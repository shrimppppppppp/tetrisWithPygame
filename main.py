...
dangThuc = pygame.image.load('dangThuc.png').convert()
...
def pickPiece():
    return random.choice([pieceI, pieceZ, pieceO])
...
def checkLimitX(piece, rotation, currentX):
    if (piece == pieceI and currentX > maxI[rotation]): 
        currentX = maxI[rotation]
    if (piece == pieceZ and currentX > maxZ[rotation]):
...
def changeRotation(piece, rotation, currentX):
    if (rotation == len(piece) - 1):
        ...
        rotation += 1
    currentX = checkLimitX(piece, rotation, currentX)
...
while True:
    ...
    currentPiece, currentRotation, currentX, currentY = checkLimitY(currentPiece, currentRotation, currentX, currentY)
    ...
    for event in pygame.event.get(): 
        if (event.type == pygame.KEYDOWN):
            if (event.key == pygame.K_LEFT):
                currentX -= 50
                currentX = checkLimitX(currentPiece, currentRotation, currentX)
                processPiece(surface, currentPiece, currentRotation, currentX, currentY)
            ...
            if (event.key == pygame.K_UP):
                currentX, currentRotation = changeRotation( currentPiece, currentRotation, currentX)
                processPiece(surface,  currentPiece, currentRotation, currentX, currentY)
